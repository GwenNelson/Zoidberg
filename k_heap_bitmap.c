...
int k_heapBMAddBlock(KHEAPBM *heap, uint64_t addr, uint64_t size, uint64_t bsize) {
	KHEAPBLOCKBM		*b;
	uint64_t				bcnt;
	uint64_t				x;
	uint8_t				*bm;
			...
			for (x = (b->lfb + 1 >= bcnt ? 0 : b->lfb + 1); x != b->lfb; ++x) {
				...
				if (bm[x] == 0) {	
					...
					if (y == bneed) {
						...
						return (void*)(uint64_t)(x * b->bsize + (uint64_t)&b[1]);
					}
...
void k_heapBMFree(KHEAPBM *heap, void *ptr) {
	...
	for (b = heap->fblock; b; b = b->next) {
		if ((uint64_t)ptr > (uint64_t)b && (uint64_t)ptr < (uint64_t)b + b->size) {
			/* found block */
